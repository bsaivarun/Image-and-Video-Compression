function [Error_Frame_Y Error_Frame_Cb Error_Frame_Cr Motion_Vector] = MotionEstimation_FullSearchAdaptive(Current_Frame_YCbCr,Ref_Frame,BlockSize,SearchRange,BinaryTree_MV, HuffCode_MV, BinCode_MV, Codelengths_MV)
                    ...
                    while(row_traverse<=SW_row_High)
                        ...
                        while(col_traverse<=SW_col_High)
                            if((col_traverse <= FrameWidth-BlockSize+1) && (row_traverse <= FrameHeight-BlockSize+1))
                                ...
                                MotionVector = Current_Frame_Block_Index-[row_traverse col_traverse];
%                                 Rate = numel(decimalToBinaryVector(abs(Current_Frame_Block_Index-[row_traverse col_traverse])));
                                Rate = enc_huffman_new(floor(MotionVector+SearchRange+2), BinCode_MV, Codelengths_MV);
                                Lagrangian_new = SSD_new+(lambda*sum(Rate));
                                if(Lagrangian_new < Lagrangian)
                                    Lagrangian = Lagrangian_new;
