...
while run <= numel(Scaling_Factors)
    ...
    RLEnc_Small = IntraEncode_YCbCr_Integer(SmallImage_YCbCr,Scaling_Factor,OptimizationScheme,EOB);

    %% Building Huffman Code
   
    FrameHist = hist(RLEnc_Small(:),-Lower:Lower);
    ...
    for iframes = 1:21
        ...
        if (iframes == 1)
            ...
            FirstFrame_zeroRun = IntraEncode_YCbCr_Integer(FirstFrame_YCbCr,Scaling_Factor,OptimizationScheme,EOB);
            ...
            [Decoded_Ref_Frame_Y, Decoded_Ref_Frame_Cb Decoded_Ref_Frame_Cr] = IntraDecode_Integer(FirstFrame_zeroRun,FirstFrame,Scaling_Factor,OptimizationScheme,EOB);
    ...
    %         Error_Frame_Cr = Error_Frame(:,:,3);
            ...
            %% Zero Run Length Encoding
            zeroRun = IntraEncode_YCbCr_Integer(Error_Frame,Scaling_Factor,OptimizationScheme,EOB);
            toc
            ...
            %% Building Huffman Code for Error Image
                Lower_ZR = -Lower;
                ImageHist_ZR = hist(zeroRun(:),Lower_ZR:1200);
            ...
            Decoded_Ref_Frame = IntraDecode_YCbCr_Integer(zeroRun,Motion_Vector,Ref_Frame,Scaling_Factor,OptimizationScheme,EOB);toc
